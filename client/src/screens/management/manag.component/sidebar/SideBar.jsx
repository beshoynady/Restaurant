            ...
            <>
              <div ref={sidebarRef} className="sidebar close" style={{ scrollbarWidth: 'thin' }}>
                ...
                <ul className="nav-links overflowX-hidden">
                  {(isProgrammer || role === 'cashier' || role === 'manager' || role === 'owner') && (
                    <li>
                      <Link to="/management">
                        <span className="material-symbols-outlined icon ml-2">dashboard</span>
                  ...
                  {/* POS */}
                  {(isProgrammer || role === 'cashier' || role === 'waiter' || role === 'manager' || role === 'owner') && (
                    <li>
                      <Link to="pos">
                        <span className="material-symbols-outlined icon ml-2">point_of_sale</span>
                        <span className="link_name">نقطة البيع</span>
                      </Link>
                      <ul className="sub-menu blank">
                        <li><Link to="pos" className="link_name">نقطة البيع</Link></li>
                      </ul>
                    </li>
                  )}

                  {/* Kitchen */}
                  {(isProgrammer || role === 'chef' || role === 'manager' || role === 'owner') && (
                    <li>
                      <Link to="kitchen">
                        <span className="material-symbols-outlined icon ml-2">cooking</span>
                  ...
                  {/* Waiter */}
                  {(isProgrammer || role === 'manager' || role === 'owner' || role === 'waiter') && (
                    <li>
                      <Link to="waiter">
                        <span className="material-symbols-outlined icon ml-2">concierge</span>
                  ...
                  {/* Deliveryman */}
                  {(isProgrammer || role === 'deliveryman' || role === 'manager' || role === 'owner') && (
                    <li>
                      <Link to="deliveryman">
                        <span className="material-symbols-outlined icon ml-2">directions_bike</span>
                  ...
                  {/* Orders */}
                  {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Orders')[0]?.read) && (
                    <li>
                      <Link to="orders">
                        <span className="material-symbols-outlined icon ml-2">list_alt</span>
                        <span className="link_name">الطلبات</span>
                      </Link>
                      <ul className="sub-menu blank">
                        <li><Link to="orders" className="link_name">الطلبات</Link></li>
                      </ul>
                    </li>
                  )}

                  {/* Tables */}
                  {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Tables')[0]?.read) && (
                    <li ref={arrowRefs.arrowtable} onClick={() => openSubMenu(arrowRefs.arrowtable)}>
                      <div className="icon-link">
                        <a href="#">
                          <span className="material-symbols-outlined icon ml-2">table_restaurant</span>
                      ...
                      <ul className="sub-menu">
                        ...
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Table Reservations')[0]?.read) && (
                  ...
                  {/* Menu */}
                  {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Products')[0]?.read) && (
                    <li ref={arrowRefs.arrowmen} onClick={() => openSubMenu(arrowRefs.arrowmen)}>
                      <div className="icon-link">
                        <a href="#">
                          <span className="material-symbols-outlined icon ml-2">restaurant_menu</span>
                      ...
                      <ul className="sub-menu">
                        ...
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Menu Categories')[0]?.read) && (
                          <li><Link to="menucategory">التصنيفات</Link></li>
                        )}
                        <li><Link to="products">الأطباق</Link></li>
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Recipes')[0]?.read) && (
                          <li><Link to="productrecipe">الوصفات</Link></li>
                        )}
                      </ul>
                    </li>
                  )}
                  ...
                  {/* Employees */}
                  {(isProgrammer || (permissionsList?.filter(permission => permission.resource === 'Employees')[0]?.read ||
                    permissionsList?.filter(permission => permission.resource === 'Permissions')[0]?.read)) && (
                      <li ref={arrowRefs.arrowemp} onClick={() => openSubMenu(arrowRefs.arrowemp)}>
                        <div className="icon-link">
                          <a href="#">
                            <span className="material-symbols-outlined icon ml-2">group_add</span>
                        ...
                        <ul className="sub-menu">
                          ...
                          {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Permissions')[0]?.read) && (
                  ...
                  {/* Users */}
                  {(isProgrammer || (permissionsList?.filter(permission => permission.resource === 'Users')[0]?.read ||
                    permissionsList?.filter(permission => permission.resource === 'Message')[0]?.read)) && (
                      <li ref={arrowRefs.arrowmessage} onClick={() => openSubMenu(arrowRefs.arrowmessage)}>
                        <div className="icon-link">
                          <a href="#">
                            <span className="material-symbols-outlined icon ml-2">user_attributes</span>
                        ...
                        <ul className="sub-menu">
                          ...
                          <li><Link to="customers">إدارة العملاء</Link></li>
                          {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Messages')[0]?.read) && (
                  ...
                  {/* Stock */}
                  {(isProgrammer || (permissionsList?.filter(permission => permission.resource === 'stock Item' || permission.resource === 'Kitchen Usage')[0]?.read)) && (
                    <li ref={arrowRefs.arrowsto} onClick={() => openSubMenu(arrowRefs.arrowsto)}>
                      <div className="icon-link">
                        <a href="#">
                          <span className="material-symbols-outlined icon ml-2">receipt_long</span>
                          <span className="link_name">المخزون</span>
                        </a>
                        <i className='bx bxs-chevron-down arrow'></i>
                      </div>
                      <ul className="sub-menu">
                        <li><a className="link_name" href="#">المخزون</a></li>
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'stock Categories')[0]?.read) && (
                          <li><Link to="categoryStock">التصنيفات</Link></li>
                        )}
                  ...
                  {/* Suppliers */}
                  {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Supplier Data')[0]?.read) && (
                    <li ref={arrowRefs.arrowssupplier} onClick={() => openSubMenu(arrowRefs.arrowssupplier)}>
                      <div className="icon-link">
                        <a href="#">
                          <span className="material-symbols-outlined icon ml-2">people</span>
                      ...
                      <ul className="sub-menu">
                        ...
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Purchases')[0]?.read) && (
                          <li><Link to="purchase">المشتريات</Link></li>
                        )}
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Purchase Returns')[0]?.read) && (
                          <li><Link to="purchasereturn">مرتجع المشتريات</Link></li>
                        )}
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Supplier Movement')[0]?.read) && (
                  ...
                  {/* Expenses */}
                  {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Expenses')[0]?.read) && (
                    <li ref={arrowRefs.arrowsexp} onClick={() => openSubMenu(arrowRefs.arrowsexp)}>
                      <div className="icon-link">
                        <a href="#">
                          <span className="material-symbols-outlined icon ml-2">money</span>
                      ...
                      <ul className="sub-menu">
                        ...
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Daily Expenses')[0]?.read) && (
                  ...
                  {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Cash Register')[0]?.read) && (
                    <li ref={arrowRefs.arrowsCash} onClick={() => openSubMenu(arrowRefs.arrowsCash)}>
                      <div className="icon-link">
                        <a href="#">
                          <span className="material-symbols-outlined icon ml-2">monetization_on</span>
                      ...
                      <ul className="sub-menu">
                        ...
                        {(isProgrammer || permissionsList?.filter(permission => permission.resource === 'Cash Movement')[0]?.read) && (
                          <li><Link to="cashmovement">تسجيل حركة</Link></li>
                        )}
                      </ul>
                    </li>
                  )}

                  {(isProgrammer || (permissionsList?.filter(permission => permission.resource === 'Delivery Zones'
                    || permission.resource === 'Shifts'
                    || permission.resource === 'Restaurant Settings')[0]?.read ))&& (
                      <li>
                        <Link to="info">
                          <span className="material-symbols-outlined icon ml-2">settings</span>
                          <span className="link_name">الإعدادات</span>
                        </Link>
                        <ul className="sub-menu blank">
                          <li><Link to="info" className="link_name">الإعدادات</Link></li>
                        </ul>
                      </li>
                    )}
