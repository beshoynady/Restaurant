...
const Waiter = () => {
  ...
  const fetchPendingData = async () => {
    try {
      ...
      const recentStatus = res.data.filter(
        (order) => order.status === "Pending"
      );
      const recentPaymentStatus = res.data.filter(
  ...
  const updateOrderDelivered = async (id) => {
    try {
      ...
      const updateOrder = await axios.put(`${apiUrl}/api/order/${id}`, {
        status,
        products,
      });
