...
const InvoiceComponent=({  ModalId, orderData, showModal, setShowModal })=>{
  ...
  return (
    <div id={ModalId} className="modal fade" style={{ display: showModal ? 'block' : 'none' }}>
          ...
          <form>
            ...
            <div ref={printContainerInvoice} className="p-1 mb-7 overflow-auto printpage" style={{ width: '100%', textAlign: 'center' }}>
              ...
              <div className="invoice-header" style={{ backgroundColor: '#343a40', color: '#ffffff', padding: '20px', textAlign: 'center' }}>
                ...
                <p>كاشير:{cashier?.username} | فاتورة #{serial} | {orderType === 'Internal' ? `طاولة' ${table?.tableNumber}` : ''} | التاريخ: {formatDateTime(new Date())}</p>
              </div>
              ...
              <table className="table table-bordered table-responsive-md" style={{ direction: 'rtl' }}>
                ...
                <tbody>
                  {products&&products.map((product, i) => (
                    <>
                      <tr key={i}>
                        <td className="col-md-3 text-truncate">{product.name}</td>
                        <td className="col-md-2 text-nowrap">{product.priceAfterDiscount ? product.priceAfterDiscount : product.price}</td>
                        <td className="col-md-2 text-nowrap">{product.quantity}</td>
                        <td className="col-md-2 text-nowrap">{product.totalprice}</td>
                      </tr>
            ...
            <div className="modal-footer d-flex flex-nowrap align-items-center justify-content-between m-0 p-1">
              ...
              <input type="submit" className="btn btn-success col-6 h-100 px-2  py-3 m-0" value="Print" onClick={handlePrintInvoice} />
            </div>
          </form>
        </div>
      </div>
