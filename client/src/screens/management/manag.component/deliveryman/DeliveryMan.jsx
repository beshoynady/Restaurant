          ...
          return (
            <div className='container-fluid d-flex flex-wrap align-content-start justify-content-around align-items-start h-100 overflow-auto bg-transparent py-5 px-3'>
              {deliveryOrders && deliveryOrders.map((order, i) => {
                ...
                const { name, serial, address, phone, deliveryMan, createdAt, updatedAt, _id: id, status, user, total, products } = order;

                return (
                  <div className="card text-white bg-success col-12 col-md-4 mb-2 p-1" key={i}>
                    <div className="card-body d-flex flex-wrap text-right d-flex justify-content-between p-1 m-1 border rounded">
                      <p className="card-text w-100 mb-1"><strong>رقم الفاتورة:</strong>
                        <a href="#invoiceOrderModal" className='h-100 btn btn-primary btn-sm' data-toggle="modal" onClick={() => getOrderDetalis(id)}>
                          {serial} </a>
                      </p>
                      <p className="card-text w-100 mb-1"><strong>الطيار:</strong> {deliveryMan?.username}</p>
                      <p className="card-text w-100 mb-1"><strong>العميل:</strong> {user ? user.username : name}</p>
                      <p className="card-text w-100 mb-1"><strong>الموبايل:</strong> {phone}</p>
                      <p className="card-text w-100 mb-1"><strong>العنوان:</strong> {address}</p>
                    </div>
                    <ul className="list-group list-group-flush text-right">
                      {products.length > 0 ? products.map((product, j) => (
                        <React.Fragment key={j}>
                          <li className={`list-group-item ${product.isAdd ? 'bg-danger text-white' : 'text-dark'} d-flex flex-column justify-content-between align-items-center`}>
                            <div className="d-flex justify-content-between align-items-center w-100">
                              <p className="mb-1" style={{ fontSize: '1.2em', fontWeight: 'bold' }}>
                                {j + 1}- {product.name} {product.size ? `- ${product.size}` : ''}
                              </p>
                              <span className="mb-1" style={{ fontSize: '1.2em', fontWeight: 'bold' }}>× {product.quantity}</span>
                            </div>
                            <div className="w-100" style={{ fontSize: '1.2em', fontWeight: 'bold' }}>{product.notes}</div>
                            {product.extras && product.extras.length > 0 && (
                              product.extras.map((extra, k) => {
                                if (extra && extra.isDone === false) {
                                  return (
                                    <div key={k} className="d-flex justify-content-start align-items-center mt-2">
                                      {extra.extraDetails.map((detail) => (
                                        <span className="badge badge-secondary m-1" key={detail.extraid}>
                                          {detail.name}
                                        </span>
                                      ))}
                                    </div>
                    ...
                    <div className="card-footer text-center text-dark">
                      <p className="mb-2 text-light" style={{ fontSize: '20px', fontWeight: '900' }}>الإجمالي: {total} ج</p>
                      {status === 'Prepared' ?
                        <button className="btn w-100 btn-primary btn-lg" onClick={() => { updateOrderOnWay(id) }}>استلام الطلب</button> :
                        <button className="btn w-100 btn-warning btn-lg" onClick={() => { updateOrderDelivered(id) }}>تم التسليم</button>
                      }
                    </div>
                  </div>
