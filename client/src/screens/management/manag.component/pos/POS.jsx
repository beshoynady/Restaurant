...
const POS = () => {
  ...
  const { restaurantData, setsalesTax, salesTax, setserviceTax, serviceTax,
    ...
    allProducts, allMenuCategories, allTable, employeeLoginInfo, setMenuCategoryId,
    deleteItemFromCart, incrementProductQuantity, decrementProductQuantity, setproductNote,
    addNoteToProduct, setitemsInCart, itemsInCart, costOrder,
    createWaiterOrderForTable, createcashierOrder,
    lastInvoiceByCashier, myOrder, listProductsOrder, orderTotal, orderSubtotal,
    ordertax, orderdeliveryCost, setdiscount, setaddition, discount,
    addition, getOrderProductForTable, itemId, addExtrasToProduct, handleAddProductExtras, productExtras,
    setproductExtras, orderDetalisBySerial, getOrderDetailsBySerial, updateOrder, productOrderToUpdate,
  ...
  const getCustomerByPhone = async (phone) => {
    ...
    try {
      const response = await axios.get(`${apiUrl}/api/customer/phone/${phone}`, config);
      ...
      if (error.response && error.response.data && error.response.data.message === 'Customer not found') {
        console.info(error);
  ...
  const createCustomer = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${apiUrl}/api/customer`, {
        name: clientname,
        phone: clientphone,
  ...
  const updateCustomer = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`${apiUrl}/api/customer/${customerId}`, {
        name: clientname,
        phone: clientphone,
        deliveryArea: deliveryAreaId,
        address: clientaddress,
  ...
  useEffect(() => {
    settotalOrder(costOrder > 0 ? costOrder + salesTax + serviceTax + deliverycost + addition - discount : 0)
  }, [costOrder, salesTax, serviceTax, deliverycost, discount, addition])
  ...
  return (
    <section className='pos-section'>
      ...
      <InvoiceComponent ModalId="invoiceModal" orderData={myOrder} showModal={showModal} setShowModal={setShowModal} />
        ...
        <div className='categ-menu'>
          ...
          <nav className='pos-category'>
            <ul className='category-ul'>
              {allMenuCategories && allMenuCategories.map((c, i) =>
                <li key={i} className='category-li' onClick={() => setMenuCategoryId(c._id)}>
              ...
              <div className="modal-body d-flex justify-content-center align-items-center ">
                <div className="w-100">
                  <div className="form-group w-100 d-flex align-item-center justify-content-between">
                    ...
                    <input type="text" id="serial" className="form-control border-primary col-6" value={serial}
          ...
          <div className="modal-content shadow-lg border-0 rounded ">
            ...
            <form className='p-1' onSubmit={(e) => { changePaymentorderstauts(e) }}>
              <div className="form-group w-100 d-flex align-items-center justify-content-between">
                ...
                <input type="text" id="totalOrder" className="form-control border-primary col-6" value={myOrder.total} readOnly />
              </div>
              <div className="form-group w-100 d-flex align-items-center justify-content-between">
              ...
              <div className="modal-body d-flex justify-content-center align-items-center ">
                <div className="w-100">
                  <div className="form-group d-flex flex-nowrap  w-100">
                    ...
                    <select id='table' className="form-control border-primary col-6 text-dark" required onChange={(e) => getOrderProductForTable(e, e.target.value)}>
          ...
          <div className="modal-content shadow-lg border-0 rounded ">
            <form>
                ...
                ordertype === 'Internal' ? (
                  <div className="modal-body d-flex justify-content-center align-items-center ">
                    <div className="form-group d-flex flex-nowrap align-items-center justify-content-between col-12 ">
                      ...
                      <select id='table' className="form-control border-primary" required onChange={(e) => { settableID(e.target.value) }}>
                        <option>اختر رقم الطاولة</option>
                        {allTable.map((table, i) => (
                          <option value={table._id} key={i}>{table.tableNumber}</option>
                        ))}
                  ...
                  <div className="modal-body d-flex justify-content-center align-items-center ">
                    ...
                    <div className="form-group d-flex flex-nowrap align-items-center justify-content-between col-12 col-md-6 ">
                      ...
                      <select id="area" className="col-8 form-control" required onChange={(e) =>{handelcustomerDeliveryArea(e.target.value)}}>
                        ...
                        {areas ? (
                          areas.map((area, i) => (
                            <option value={JSON.stringify(area)} key={i}>{area.name}</option>
                          ))
                        ) : (
                    ...
                    {isClientFounded ?
                      <>
                        <div className="form-group w-100 h-auto px-3 d-flex align-items-center justify-content-start col-12 col-md-6">
                          ...
                          <select name='isVarified' required className="form-control border-primary"   value={isVarified} onChange={(e) => setisVarified(e.target.value)}>
                        ...
                        <div className="form-group w-100 h-auto px-3 d-flex align-items-center justify-content-start col-12 col-md-6">
                          ...
                          <select name='refusesOrders' required className="form-control border-primary"   value={refusesOrders} onChange={(e) => setrefusesOrders(e.target.value)}>
                    ...
                    <div className="form-group w-100 h-auto px-3 d-flex align-items-center justify-content-start col-12 col-md-6">
                      ...
                      <textarea className="form-control border-primary" name='notes' value={clientNotes} onChange={(e) => setclientNotes(e.target.value)} />
                      <div className="invalid-feedback">الرجاء إدخال ملاحظات صالحة.</div>
                    </div>

                    {!isClientFounded ?
                      <button type="button" className="h-100 btn btn-success col-12 col-md-6" id="customerInfo"
                        onClick={createCustomer}> حفظ بيانات العميل </button>

                      :
                      <button type="button" className="h-100 btn btn-success col-12 col-md-6" id="customerInfo"
                        onClick={updateCustomer}> تعديل بيانات العميل </button>
                    }
                  </div>
              ...
              <div className="modal-footer d-flex flex-nowrap align-items-center justify-content-between m-0 p-1">
                ...
                <input type="button" className="btn btn-danger col-6 h-100 p-0 m-0" data-dismiss="modal" value="اغلاق" onClick={() => { deleteOrderdetalis() }} />
              </div>
            </form>
          </div>
        </div>
      ...
      <div className="container-fluid d-flex flex-column justify-content-between align-items-stretch align-content-between flex-nowrap " style={{ width: '450px', height: '100%', padding: '0', margin: '0' }}>
        <div className="row w-100 p-0 m-0">
          <div className="btn-group col-12 p-0 m-0" role="group" aria-label="Order Type" style={{ height: '50px' }}>
            <a href="#typeOrderModal" className="btn d-flex align-items-center justify-content-center btn-primary col-4 p-0 m-0" data-toggle="modal" onClick={() => { setordertype('Internal') }} style={{ height: '50px' }}>
              الصالة
            </a>
            <a href="#typeOrderModal" className="btn d-flex align-items-center justify-content-center btn-success col-4 p-0 m-0" data-toggle="modal" onClick={() => { setordertype('Takeaway') }} style={{ height: '50px' }}>
              التيك أوي
            </a>
            <a href="#typeOrderModal" className="btn d-flex align-items-center justify-content-center btn-danger col-4 p-0 m-0" data-toggle="modal" onClick={() => { setordertype('Delivery') }} style={{ height: '50px' }}>
              التوصيل
            </a>
          </div>
        </div>
                      ...
                      (<div className="position-absolute w-100 h-auto top-0 start-0 bg-white rounded-3 d-flex flex-column align-items-center justify-content-center overflow-hidden"
                        ...
                        <form onSubmit={(e) => { if (product.extras.length > 0) { addExtrasToProduct(e, product._id, item.sizeId); }; setSelectedButtonIndex(1); setextraArea(!extraArea); }}
                                ...
                                <div key={ind} className="form-group w-100 h-100 d-flex align-items-start justify-content-start flex-wrap" style={{ padding: '5px', overflowY: "auto" }}>
                                  {product.extras.map((extra, i) => (
                                    <div className="form-check form-check-flat mb-1 d-flex align-items-center" key={i} style={{ width: '47%', paddingLeft: '5px' }}>
                                      ...
                                      />
                                      <label className="form-check-label text-dark mr-4" style={{ fontSize: '14px', fontWeight: '900' }} onClick={(e) => handleAddProductExtras(extra, ind)}>
                          ...
                          <div className="note-btn d-flex align-items-center justify-content-center w-100 mt-2" style={{ height: '40px' }}>
                            <button className="h-100 btn btn-success rounded-2 me-2" style={{ width: '50%' }}>تأكيد</button>
                            <button type="button" onClick={() => setextraArea(!extraArea)} className="h-100 btn btn-danger rounded-2" style={{ width: '50%' }}>إلغاء</button>
                          </div>
                        </form>
                      </div>
                      )
                      : product && !item.sizeId && product._id && product._id === item.productid && extraArea === true && product.quantity > 0 ?
                        (<div className="position-absolute w-100 h-auto top-0 start-0 bg-white rounded-3 d-flex flex-column align-items-center justify-content-center overflow-hidden"
                          style={{ zIndex: 10 }}>
                  ...
                  {/* noteArea */}
                  {
                    product && item.sizeId && sizeId === item.sizeId && noteArea ? (
                      <div className="position-absolute w-100 h-100 top-0 start-0 bg-white rounded-3 d-flex flex-column align-items-center justify-content-center overflow-hidden"
                        style={{ zIndex: 10 }}>
                        <form className="card-body w-100 h-100 p-1 m-0" onSubmit={(e) => { addNoteToProduct(e, item.productid, item.sizeId); setnoteArea(!noteArea) }}>
                          <textarea className="form-control w-100 h-75 p-1 m-0" defaultValue={item.notes} placeholder='اضف تعليماتك الخاصة بهذا الطبق' name='note'
                    ...
                    <div className="d-flex justify-content-between align-items-center py-2">
                      ...
                      {item.hasExtras &&
                        <span className="material-icons" style={{ color: "green", fontSize: "45px", cursor: 'pointer' }}
                          onClick={() => {
                            setproductExtras(item.extras); setextraArea(!extraArea);
                            getProductDitalis(allProducts, item.productid);
                            item.sizeId ? setsizeId(item.sizeId)
                              : setproductid(item.productid)
                          }}
                        >add_circle</span>
                      }

                      <button onClick={() => deleteItemFromCart(item.productid, item.sizeId)} className="btn btn-danger col-3 h-100 p-0 m-0">حذف</button>
                    </div>
                    <div className="d-flex justify-content-between align-items-center py-2">
                      <div className="fw-bold w-25 text-center"> {item.priceAfterDiscount ? item.priceAfterDiscount : item.price} ج</div>
                      <div className="w-50 d-flex justify-content-between">
                        <button onClick={() => decrementProductQuantity(item.productid, item.sizeId)} className="btn col-4 h-100 p-0 m-0 btn-primary">-</button>
                        <span className="text-dark fw-bold fs-5 text-center">{item.quantity > 0 ? item.quantity : 0}</span>
                        <button onClick={() => incrementProductQuantity(item.productid, item.sizeId)} className="btn col-4 h-100 p-0 m-0 btn-primary">+</button>
                      </div>
                    ...
                    {item.extras && (
                      <div className="d-flex flex-columen flex-wrap mt-2">
                        {item.extras.map((extra, i) => (
                          extra && extra.totalExtrasPrice > 0 &&
        ...
        <div className='h-auto w-100'>
          <div className="row d-flex align-items-start px-2 m-0" style={{ direction: 'rtl' }}>
            <div className="col p-0 m-0">
              <div className="order-details bg-white border rounded shadow-sm p-0 mb-1">
                <p className="order-item mb-1 d-flex justify-content-between align-items-center text-black">
                  ...
                  <span className="text-dark fw-bold fs-5 text-center">{costOrder > 0 ? costOrder : 0} ج</span>
                </p>
                {ordertype === 'Internal' && restaurantData.serviceTaxRate > 0 ? (
                  <p className="order-item mb-1 d-flex justify-content-between align-items-center text-black">
                    <span className="text-dark fw-bold fs-5">{`خدمة صاله ${restaurantData.serviceTaxRate}%:`}</span>
                    <span className="text-dark fw-bold fs-5 text-center">{serviceTax > 0 ? serviceTax : 0} ج</span>
                  </p>
                ) : null}
                {restaurantData.salesTaxRate > 0 ? (
                  <p className="order-item mb-1 d-flex justify-content-between align-items-center text-black">
                    <span className="text-dark fw-bold fs-5">{`ضريبة مبيعات ${restaurantData.salesTaxRate}%:`}</span>
                    <span className="text-dark fw-bold fs-5 text-center">{salesTax > 0 ? salesTax : 0} ج</span>
                  </p>
                ) : null}
                {ordertype === 'Delivery' ? (
                  <p className="order-item mb-1 d-flex justify-content-between align-items-center text-black">
                    <span className="text-dark fw-bold fs-5">خدمة التوصيل:</span>
                    <span className="text-dark fw-bold fs-5 text-center">{deliverycost > 0 ? deliverycost : 0} ج</span>
                  </p>
                ) : null}
                ...
                <p className="order-item border-bottom m-0 d-flex justify-content-between align-items-center text-black">
                  ...
                  <span className="text-dark fw-bold fs-5 text-center">{totalOrder ? totalOrder : 0} ج</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="d-flex flex-wrap g-2 align-items-center justify-content-between w-100 p-0 m-0">
            {ordertype === 'Internal' ?
              <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-primary"
                onClick={() => {
                  createWaiterOrderForTable(tableID, employeeLoginInfo.id);
                  setaddaddition(false); setadddiscount(false);
                }}>تأكيد</button>

              : ordertype === 'Delivery' ?
                <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-primary"
                  onClick={() => {
                    createcashierOrder(employeeLoginInfo.id, clientname, clientphone, clientaddress,
                      ordertype, deliverycost, discount, addition);
                    setaddaddition(false); setadddiscount(false);
                  }}>تأكيد</button>
                ...
                : ordertype === 'Takeaway' ?
                  ...
                  : <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-primary"
                    onClick={() => alert('اختر نوع الاوردر و اكتب جميع البيانات')}>تأكيد</button>
            }
            {productOrderToUpdate.length > 0 ?
              <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-warning" onClick={() => updateOrder()}>عدل</button>
              : <a type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-warning" href="#getOrderDetalisModal" data-toggle="modal">تعديل</a>
            }

            <a type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn bg-success" href="#paymentModal" data-toggle="modal" onClick={() => { lastInvoiceByCashier(employeeLoginInfo.id); getCashRegistersByEmployee(employeeLoginInfo.id) }}>دفع</a>
            <a type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-info" href="#getOrderTableModal" data-toggle="modal">دفع جزء</a>
            <a type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-success" data-toggle="modal" data-target="#invoiceModal"
              onClick={() => { lastInvoiceByCashier(employeeLoginInfo.id); setShowModal(!showModal); }}>طباعة</a>
            <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-secondary" onClick={() => setaddaddition(!addaddition)}>رسوم</button>
            <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-secondary" onClick={() => setadddiscount(!adddiscount)}>خصم</button>
            <button type="button" style={{ height: '60px' }} className="col-3 d-flex align-items-center justify-content-center text-nowrap btn btn-danger" onClick={() => { setitemsInCart([]); deleteOrderdetalis(); }}>إلغاء </button>
          </div>
        </div>
