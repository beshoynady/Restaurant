...
const MenuCard = () => {
  ...
  return (
    <detacontext.Consumer>
      {
        ({ allProducts, MenuCategoryId, addItemToCart, deleteItemFromCart, incrementProductQuantity, setproductExtras, decrementProductQuantity,
           setproductNote, addNoteToProduct, addExtrasToProduct, handleAddProductExtras, productExtras, itemId }) => {
          return (

            <div className="d-flex flex-wrap flex-md-row">
              {allProducts.length > 0 ?
                allProducts.filter(pro => pro.category._id === MenuCategoryId).map((product, index) => {
                  if (product.hasSizes) {
                    return (
                        ...
                        <div className="row g-0 h-100 m-0 p-0">
                            ...
                            {product.available === true ? (
                              <>
                                {itemId.includes(sizeId) && sizeId && product.sizes.filter(size => size._id === sizeId)[0]?.sizeQuantity > 0 ? (
                                  <button type="button" className="h-25 p-0 m-0 btn btn-danger btn-block" style={{ fontSize: "14px" }} onClick={() => { deleteItemFromCart(product._id, sizeId) }}>
                                    حذف من الطلبات
                                  </button>
                                ) : (
                                  <button type="button" className="h-25 p-0 m-0 btn btn-success btn-block" style={{ fontSize: "14px" }} onClick={() => { if (sizeQuantity > 0) { addItemToCart(product._id, size._id) } }}>
                                    أضف الى طلباتي
                                  </button>
                                )}
                              </>
                            ) : (
                              <button type="button" className="h-25 p-0 m-0 btn btn-warning btn-block" style={{ fontSize: "14px" }}>
                                غير متاح الان
                              </button>
                            )}
                          ...
                          <div className="col-7 d-flex flex-column justify-content-between align-items-stretch p-1">
                            ...
                            <div className="d-flex row justify-content-between align-items-center mb-2">
                              <div className="col-8 btn-group btn-group-toggle" style={{ direction: 'ltr' }} data-toggle="buttons">
                                {product.sizes.length > 0 && product.sizes?.map((size, i) => {
                                  return (
                                    <label key={i} className={`d-flex justify-content-center align-items-center col-4 h-100 btn btn-outline-secondary btn-sm${size._id === sizeId ? "btn-info" : 'btn-primary'}`} style={{ height: "40px", fontSize: "24px", fontWeight: "600" }} 
                                    defaultChecked={size._id === sizeId ? true : i === 0 ? true : false} onClick={() => handleSelectSize(size)}>
                            ...
                            <div className="form-row align-items-center">
                              <div className="col-4">
                                <button className="h-100 btn btn-outline-secondary w-100" type="button" onClick={() => { incrementProductQuantity(product._id, sizeId); setsizeQuantity(sizeQuantity + 1) }}>+</button>
                              </div>
                              <div className="col-4">
                              ...
                              <div className="col-4">
                                <button className="h-100 btn btn-outline-secondary w-100" type="button" onClick={() => { decrementProductQuantity(product._id, sizeId); setsizeQuantity(sizeQuantity - 1) }}>-</button>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="card mx-auto" key={index} style={{ maxWidth: "320px", minWidth: '270px', width: "100%", height: '200px', margin: '0 0 10px 10px' }}>
                        ...
                        <div className="row g-0 h-100 p-0 m-0">
                            ...
                            {product.available === true ? (
                              itemId.filter((i) => i === product._id).length > 0 && product.quantity > 0 ?
                                (
                                  ...
                                  <button type="button" className="h-25 p-0 m-0 btn btn-success btn-block" style={{ fontSize: "14px" }} onClick={() => { if (product.quantity > 0) { addItemToCart(product._id) } }}>
                                    أضف الى طلباتي
                                  </button>
                                )
                            ) : (
                          ...
                          <div className="col-7 d-flex flex-column justify-content-between align-items-stretch p-1">
                            ...
                            <div className="form-row align-items-center">
                              <div className="col-4">
                                <button className="h-100 btn btn-outline-secondary w-100" type="button" onClick={() => incrementProductQuantity(product._id)}>+</button>
                              </div>
                              <div className="col-4">
                                <input type="text" className="form-control text-center w-100" readOnly value={product.quantity} />
                              </div>
                              <div className="col-4">
                                <button className="h-100 btn btn-outline-secondary w-100" type="button" onClick={() => decrementProductQuantity(product._id)}>-</button>
                              </div>
                            </div>
                          </div>
                        </div>
