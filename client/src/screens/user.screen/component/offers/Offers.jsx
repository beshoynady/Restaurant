                              ...
                              {size._id === productid && noteArea === true ? (
                                ...
                                >
                                  <textarea
                                    ...
                                    style={{ zIndex: 11 }}
                                  ></textarea>
                                  <div className='d-flex align-items-center justify-content-center w-100 mt-2' style={{ height: '40px' }}>
                                    <button type="submit" className="btn btn-success w-50 h-100 me-2">تاكيد</button>
                                    <button type="button" onClick={() => setnoteArea(!noteArea)} className="btn btn-danger w-50 h-100">الغاء</button>
                                  </div>
                                </form>
                              ) : ''}
                                                ...
                                                <div className=" mb-1 d-flex align-items-center justify-content-between col-6 pl-1" key={i}>
                                                  <input
                                                    ...
                                                    defaultChecked={
                                                      ...
                                                      (product.sizes.filter(size => size._id === size._id)[0].extrasSelected &&
                                                        product.sizes.filter(size => size._id === size._id)[0].extrasSelected[ind] &&
                                                        product.sizes.filter(size => size._id === size._id)[0].extrasSelected[ind].extraDetails.some(detail => detail.extraId === extra._id))
                                                    }
                                                    onChange={(e) => handleAddProductExtras(extra, ind)}
                                                  />
                                                  <label className="ms-2" style={{ fontSize: '14px', fontWeight: '900' }} onClick={(e) => handleAddProductExtras(extra, ind)}>{`${extra.name} - ${extra.price} ج`} </label>
                                                </div>
                                              ))}
                                            </div>
                                          )
                              ...
                              <div className="card-body d-flex flex-column flex-nowrap align-items-center justify-content-between w-100 p-0 m-0" style={{height:'60%'}}>
                                ...
                                <div className="w-100 p-1 m-0 d-flex justify-content-between align-items-center">
                                  ...
                                  <div style={{ fontSize: '18px', fontWeight: '600'}}>
                                    ...
                                    <sup><del style={{ fontSize: '14px' }}>{size.sizePrice}</del></sup>
                                  </div>
                                </div>
                                <div className="w-100 h-25 p-0 mt-2">
                                  {itemId.filter((i) => i === size._id).length > 0 && size.sizeQuantity > 0 ? (
                                    <button className="btn btn-danger w-100 h-100 m-0" onClick={() => { deleteItemFromCart(product._id, size._id); }}>
                                      احذف من الطلبات
                                    </button>
                                  ) : (
                                    <button className="btn btn-success w-100 h-100 m-0" onClick={() => { if (size.sizeQuantity > 0) { addItemToCart(product._id, size._id) } }}>
                                      اضف الي طلباتي
                                    </button>
                                  )}
                                </div>
