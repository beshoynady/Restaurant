...
const getSupplierById = async (req, res) => {
    try {
        ...
        const supplier = await SupplierModel.findById(supplierId).populate('itemsSupplied').populate('createdBy');
        if (!supplier) {
        ...
        res.status(500).json({ message: 'Failed to get supplier', error: error.message });
    }
}
