...
const createCustomerMessage = async (req, res) => {
  try {
    ...
    const newMessage = await CustomerMessageModel.create({
      name,
      email,
      phone,
      message,
...
const updateCustomerMessageById = async (req, res) => {
  try {
    ...
    const updatedMessage = await CustomerMessageModel.findByIdAndUpdate(
      messageId,
      { isSeen },
      { new: true }
    );
...
const deleteCustomerMessageById = async (req, res) => {
  try {
    const messageId = req.params.id;
    const deletedMessage = await CustomerMessageModel.findByIdAndDelete(
      messageId
    );
    if (!deletedMessage) {
      return res.status(404).json({ message: "Message not found" });
