...
const createMenuCategory = async (req, res, next) => {
  try {
    ...
    if (isExist) {
      return res
        .status(400)
        .json({ message: "Menu category name already exists" });
    }
    ...
    if (error.code === 11000) {
      ...
      return res
        .status(400)
        .json({ message: "menuCategory name already exists", error });
    }
    // Handle other errors
...
const getAllMenuCategories = async (req, res, next) => {
  try {
    const allMenuCategories = await menuCategoryModel
      .find({})
      .sort("order")
      .populate("createdBy");
...
const updateMenuCategory = async (req, res, next) => {
  ...
  try {
    ...
    if (error.code === 11000) {
      ...
      return res
        .status(400)
        .json({ message: "menuCategory name already exists" });
    }
    // Handle other errors
...
const deleteMenuCategory = async (req, res, next) => {
  const { menuCategoryId } = req.params;
  try {
    const deletedmenuCategory = await menuCategoryModel.findByIdAndDelete(
      menuCategoryId
    );
