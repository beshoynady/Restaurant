const DeliveryAreaModel = require("../models/delivery-area.model");

// Create a new delivery area
const createDeliveryArea = async (req, res) => {
  try {
    const { name, delivery_fee } = req.body;
    const newDeliveryArea = await DeliveryAreaModel.create({
      name,
      delivery_fee,
    });
...
const getAllDeliveryAreas = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get delivery areas", error: error.message });
  }
};
...
const getDeliveryAreaById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to get delivery area", error: error.message });
  }
};
...
const updateDeliveryArea = async (req, res) => {
  try {
    const deliveryAreaId = req.params.id;
    const { name, delivery_fee } = req.body;
    const updatedDeliveryArea = await DeliveryAreaModel.findByIdAndUpdate(
      deliveryAreaId,
      { name, delivery_fee },
      { new: true }
    );
...
const deleteDeliveryArea = async (req, res) => {
  try {
    const deliveryAreaId = req.params.id;
    const deletedDeliveryArea = await DeliveryAreaModel.findByIdAndDelete(
      deliveryAreaId
    );
